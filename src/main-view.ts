...
const RATIO = 1.5;
const IMMERSIVE_FOV = 34.5;
const IMMERSIVE_CAMERA_Y = -World.WIDTH * 0.9;
const IMMERSIVE_CAMERA_Z = World.WIDTH * 0.64;
const IMMERSIVE_TARGET_Y = -World.WIDTH * 0.04;
const IMMERSIVE_LOOK_DOWN_Y = World.WIDTH * 0.16;
...
export class MainView {
  ...
  constructor(mainGroup: Group) {
    ...
    this.stats.dom.classList.add('perf-stats');
  ...
  private setupLights(): void {
    this.viewGroup.add(new AmbientLight(0xe5f0ea, 1.36));

    const topLight = new DirectionalLight(0xfffbf0, 1.32);
    topLight.position.set(-2500, -3500, 10000);
    this.viewGroup.add(topLight);

    const frontLight = new DirectionalLight(0xf3fbff, 0.52);
    frontLight.position.set(0, -10000, 2500);
    this.viewGroup.add(frontLight);

    const sideLight = new DirectionalLight(0xaed8f2, 0.44);
    sideLight.position.set(-10000, -7000, 2600);
    this.viewGroup.add(sideLight);

    const rimLight = new DirectionalLight(0xffe0a6, 0.16);
    rimLight.position.set(9000, 6500, 2400);
    this.viewGroup.add(rimLight);
  }

  private setupRendering(): void {
    const w = this.width || this.renderer.domElement.clientWidth;
    const h = this.height || this.renderer.domElement.clientHeight;
  ...
  private makeCamera(perspective: boolean): Camera {
    if (perspective) {
      const camera = new PerspectiveCamera(IMMERSIVE_FOV, this.aspect, 0.1, 1000);
      return camera;
    } else {
      let w: number;
      let h: number;
      if (this.aspect >= RATIO) {
        h = World.WIDTH * 0.8;
        w = h * this.aspect;
      } else {
        w = World.WIDTH * 1.2;
    ...
    mouse2: Vector2 | null): void
  {
    if (fromTop) {
      ...
      this.camera.position.set(0, IMMERSIVE_CAMERA_Y, IMMERSIVE_CAMERA_Z);
      const target = new Vector3(0, IMMERSIVE_TARGET_Y - lookDown * IMMERSIVE_LOOK_DOWN_Y, 0);
      this.viewGroup.localToWorld(target);
  ...
  updateViewport(): void {
    if (this.main.parentElement!.clientWidth !== this.width ||
      ...
      const nextAspect = renderWidth / renderHeight;
      const cameraNeedsUpdate = Math.abs(nextAspect - this.aspect) > 0.001;
